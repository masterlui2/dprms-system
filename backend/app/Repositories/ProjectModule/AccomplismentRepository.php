...
class AccomplismentRepository extends BaseRepository implements AccomplishmentRepositoryInterface{
    ...
    public function updateMany(int $summaryId, array $rows): Collection
    {
        return Collection::make($rows)->map(function (array $rows) use ($summaryId) {
            $id = $rows['id'];
            $accomplishment =  $this->model->newQuery()->where('executive_summary_id',$summaryId)->find($id);
            if (! $accomplishment){
                abort(404,"Not Found");
            }

            $accomplishment->fill(collect($rows)->except('id')->toArray());
            $accomplishment->save();
