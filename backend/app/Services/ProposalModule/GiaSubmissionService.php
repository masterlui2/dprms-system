...
use App\Services\ProposalModule\GiaProposalPDFService;
...
class GiaSubmissionService implements GiaSubmissionServiceInterface
{
    public function __construct(
        protected ProposalServiceInterface $proposalService,
        protected GiaProposalServiceInterface $giaProposalService,
        protected DocumentsServiceInterface $documentsService,
        protected GiaProposalPDFService $pdfService,
    ) {
    }
    ...
    public function submit(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            try {
                ...
                $giaProposal = $this->giaProposalService->createGiaProposal(array_merge(
                    $data,
                    [
                        'proposal_id' => $proposal->id,
                        // gia_proposals stores these under research_type /
                        // research_category (enum columns matching the same
                        // value lists as project_type / project_category
                        // respectively) rather than the request's field
                        // names — map them here so GiaProposal::create()
                        // actually receives them via $fillable.
                        'research_type' => $data['project_type'],
                        'research_category' => $data['project_category'],
                    ]
                ));
                ...
                if (! $documentType) {
                    throw new \RuntimeException('GIA proposal DocumentType (set_number=PROPOSAL) is not seeded.');
                }
                ...
                // SubmitGiaProposalRequest — every is_required DocumentType
                // for this applicable_gia_categories must be present).
    ...
    protected function buildProposalPdfUploadedFile(array $data): UploadedFile
    {
        ...
        $tempPath = tempnam(sys_get_temp_dir(), 'gia_proposal_');
        file_put_contents($tempPath, $pdfBinary);

        $fileName = 'gia-proposal-' . now()->format('Ymd-His') . '.pdf';
