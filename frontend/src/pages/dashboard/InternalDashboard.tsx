...
function dashboardContent(role: UserRole): {
  ...
  const pendingApplications = proposalRecords.filter((proposal) => proposal.status === 'Pending' || proposal.status === 'Under review').length
  ...
  switch (role) {
    case ROLES.PROJECT_STAFF:
      return {
        ...
        cards: [
          { label: 'Data Entry', value: String(pendingApplications), detail: 'Applications for encoding', icon: ClipboardCheck, tone: 'blue' },
    ...
    case ROLES.FOCAL:
      return {
        ...
        cards: [
          { label: 'For Review', value: String(pendingApplications), detail: 'Uploaded proposals to evaluate', icon: ClipboardCheck, tone: 'blue' },
    ...
    case ROLES.PROVINCIAL_DIRECTOR:
      return {
        ...
        cards: [
          { label: 'Final Decisions', value: '3', detail: 'Awaiting action', icon: ShieldCheck, tone: 'blue' },
