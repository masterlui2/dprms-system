/**
 * System: DPRMS
 * Purpose: Render programs section for the frontend.
...
import { getProgramRegistrationUrl } from '../../lib/program_access';
    ...
    title: string;
        ...
        },
        {
            ...
            items: [
                ...
                {
                    icon: FileText,
                    label: 'Project proposal, budget, and endorsements are usually needed',
                },
            ],
            label: 'GIA',
...
/** Render programs section and its available actions. */
            ...
            <div className="mx-auto max-w-6xl">
                <div className="grid gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.35fr)] lg:items-end">
                    ...
                    <p className="max-w-2xl text-base leading-7 text-slate-600 lg:justify-self-end">
                        Start with the track that matches your applicant type and project purpose.
                        Each page explains who it is for, what to prepare, and how to continue with
                        the form.
                    </p>
                </div>

                <div className="mt-8 grid gap-5 lg:grid-cols-2">
                    {PROGRAMS.map((objProgram) => (
                        <ProgramCard key={objProgram.label} objProgram={objProgram} />
                    ))}
                </div>
            </div>
        </section>
...
/** Render program card and its available actions. */
function ProgramCard({ objProgram }: { objProgram: (typeof PROGRAMS)[number]; })
{
    const Icon = objProgram.icon;

    return (
        <article className="flex h-full flex-col overflow-hidden rounded-lg border border-[#d8e5f2] bg-white shadow-sm">
            <div className={`h-2 ${objProgram.accent}`} />
            <div className="flex flex-1 flex-col p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                    <div
                        className={`grid size-12 place-items-center rounded-lg ${objProgram.accent} text-white`}
                    >
                ...
                <p className="mt-3 text-sm leading-7 text-slate-600">{objProgram.description}</p>

                <div className="mt-5 grid gap-2">
                    {objProgram.items.map(({ icon: ItemIcon, label: strLabel }) => (
                        <div
                ...
                <div className="mt-auto pt-6">
                    <Link
                        ...
                        to={objProgram.detailsTo}
                    >
                        {objProgram.linkLabel}
