        ...
        <section>
          ...
          <div className="mt-3 hidden overflow-hidden rounded-xl border border-slate-200 lg:block">
            <table className="w-full table-fixed border-collapse text-left text-xs">
              ...
              <thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr>{['Item No.', 'Equipment Name / Description', 'Qty', 'Unit', 'Amount', 'Property Number', 'Remarks'].map((header) => <th className={cn('border-r border-slate-200 px-3 py-2.5 font-semibold last:border-r-0', ['Item No.', 'Qty', 'Amount'].includes(header) && 'text-right tabular-nums')} key={header}>{header}</th>)}</tr></thead>
              <tbody><tr className="border-t border-slate-200 text-slate-700"><td className="border-r border-slate-200 px-3 py-3 text-right tabular-nums">1</td><td className="border-r border-slate-200 px-3 py-3"><span className="block font-semibold text-slate-900">{asset.name}</span><span className="mt-0.5 block text-[11px] text-slate-500">{[asset.brand, asset.model, asset.serialNumber ? `SN ${asset.serialNumber}` : null].filter(Boolean).join(' · ')}</span></td><td className="border-r border-slate-200 px-3 py-3 text-right tabular-nums">1</td><td className="border-r border-slate-200 px-3 py-3">{asset.unit || 'unit'}</td><td className="border-r border-slate-200 px-3 py-3 text-right font-medium tabular-nums">{formatMoney(asset.acquisitionCost)}</td><td className="break-words border-r border-slate-200 px-3 py-3 font-mono text-[11px]">{asset.propertyNumber || asset.id}</td><td className="px-3 py-3 font-medium">{selectedCondition}</td></tr></tbody>
            </table>
          </div>
