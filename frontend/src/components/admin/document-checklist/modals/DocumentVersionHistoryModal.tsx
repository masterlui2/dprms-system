/**
 * System: DPRMS
 * Purpose: Render document version history modal for the frontend.
...
{
    objVersionModalDoc: DocumentChecklistItem | null;
    setVersionModalDoc: (objItem: DocumentChecklistItem | null) => void;
    handlePreviewDocument: (objItem: DocumentChecklistItem) => void;
    handleOpenReviewModal: (objItem: DocumentChecklistItem) => void;
    handleRestoreArchivedVersion: (objItem: DocumentChecklistItem, objArchived: any) => void;
}

/** Render document version history modal and its available actions. */
...
{
    if (!objVersionModalDoc || !objVersionModalDoc.uploadedDoc)
    {
        return null;
    }

    const arrArchivedVersions = objVersionModalDoc.uploadedDoc.archived_versions ?? [];
    const blnHasVersionHistory = arrArchivedVersions.length > 0;
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">
                                Audit trail of current and superseded submissions
                            </p>
                        </div>
                    </div>
                ...
                <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3.5 space-y-1">
                        ...
                        <p className="text-sm font-bold text-slate-950 leading-snug">
                            {objVersionModalDoc.name}
                        </p>
                        <p className="text-[11px] text-slate-500">{objVersionModalDoc.group}</p>
                    </div>
                    ...
                    <div className="rounded-2xl border-2 border-emerald-200 bg-emerald-50/40 p-4 space-y-2">
                        <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 min-w-0">
                                <span className="rounded-md bg-emerald-600 px-2 py-0.5 text-[10px] font-bold text-white uppercase shrink-0">
                                    Current Version (V
                                    {(objVersionModalDoc.uploadedDoc.archived_versions?.length ||
                                        0) + 1}
                                    )
                                </span>
                                <span className="text-xs font-bold text-slate-900 truncate">
                                    {objVersionModalDoc.uploadedDoc.file_name}
                                </span>
                            </div>
                            <button
                                type="button"
                                onClick={() =>
                                {
                                    const objDoc = objVersionModalDoc;
                                    setVersionModalDoc(null);
                                    handlePreviewDocument(objDoc);
                                }}
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-emerald-100 font-mono">
                            <span>
                                Size: {formatFileSize(objVersionModalDoc.uploadedDoc.file_size)}
                            </span>
                            <span>
                                Uploaded:{' '}
                                {formatRelativeDate(
                                    objVersionModalDoc.uploadedDoc.created_at ||
                                    objVersionModalDoc.uploadedDoc.updated_at,
                                )}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                        ...
                        {blnHasVersionHistory ? (
                            arrArchivedVersions.map((objArchived, intAIdx) => (
                                <div
                                    key={objArchived.id || intAIdx}
                                ...
                                >
                                    <div className="flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                            ...
                                            <span className="rounded-md bg-slate-200/80 text-slate-700 px-2 py-0.5 text-[10px] font-bold uppercase shrink-0 font-mono">
                                                v{objArchived.version || intAIdx + 1}
                                            </span>
                                            <span
                                                className="text-xs font-bold text-slate-800 truncate"
                                                title={objArchived.file_name}
                                            >
                                                {objArchived.file_name}
                                            </span>
                                        </div>
                                            ...
                                            <button
                                                ...
                                                onClick={() =>
                                                {
                                                    const objTempItem = {
                                                        ...objVersionModalDoc,
                                                        uploadedDoc: {
                                                            ...objVersionModalDoc.uploadedDoc!,
                                                    ...
                                                    setVersionModalDoc(null);
                                                    handleOpenReviewModal(objTempItem);
                                                }}
                                    ...
                                    <div className="flex items-center justify-between text-[10px] text-slate-500 pt-1 border-t border-slate-200/70 font-mono">
                                        <span>Size: {formatFileSize(objArchived.file_size)}</span>
                                        <span>
                                            Archived:{' '}
                                            {formatRelativeDate(
                                                objArchived.archived_at || objArchived.created_at,
                                            )}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <p className="text-xs text-slate-400 italic">
                                No previous versions on record.
                            </p>
                        )}
                    </div>
                </div>
...
} /* end DocumentVersionHistoryModal */
