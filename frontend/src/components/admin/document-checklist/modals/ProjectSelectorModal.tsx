/**
 * System: DPRMS
 * Purpose: Render project selector modal for the frontend.
...
{
    blnIsProjectSelectorOpen: boolean;
    setIsProjectSelectorOpen: (blnOpen: boolean) => void;
    strActiveProgram: string;
    strModalSearchQuery: string;
    setModalSearchQuery: (strQuery: string) => void;
    strModalFilter: 'ALL' | 'COMPLETE' | 'INCOMPLETE';
    setModalFilter: (strFilter: 'ALL' | 'COMPLETE' | 'INCOMPLETE') => void;
    arrFilteredModalProposals: ProposalChecklistRecord[];
    objActiveProposal: ProposalChecklistRecord | null;
    handleSelectProposal: (objProposal: ProposalChecklistRecord) => void;
}

/** Render project selector modal and its available actions. */
export function ProjectSelectorModal({
    blnIsProjectSelectorOpen,
    ...
    strModalFilter,
    setModalFilter,
    arrFilteredModalProposals,
    objActiveProposal,
    handleSelectProposal,
}: ProjectSelectorModalProps)
{
    if (!blnIsProjectSelectorOpen)
    {
        return null;
    }
            ...
            <div className="flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl border border-[#B5BFCD]/60 animate-in zoom-in-95 duration-150">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">
                                Choose a {strActiveProgram} project to inspect its document
                                checklist
                            </p>
                        </div>
                    </div>
                ...
                <div className="border-b border-[#B5BFCD]/40 bg-white p-4 space-y-3">
                    <div className="relative">
                        ...
                        <input
                            type="search"
                            value={strModalSearchQuery}
                            onChange={(objEvent) => setModalSearchQuery(objEvent.target.value)}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {(['ALL', 'COMPLETE', 'INCOMPLETE'] as const).map((strFilter) => (
                            <button
                                key={strFilter}
                                type="button"
                                onClick={() => setModalFilter(strFilter)}
                                className={cn(
                                    'rounded-lg px-2.5 py-1 text-xs font-bold transition cursor-pointer',
                                    strModalFilter === strFilter
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2.5 divide-y divide-slate-100">
                    {arrFilteredModalProposals.length === 0 ? (
                        <div className="flex min-h-48 flex-col items-center justify-center p-6 text-center text-slate-400">
                            ...
                            <p className="text-[11px] text-slate-500">
                                Try searching for a different keyword or change filter.
                            </p>
                        </div>
                    ) : (
                        arrFilteredModalProposals.map(
                            (objProposal) =>
                            {
                                const blnIsComplete = objProposal.compliancePercentage >= 100;
                                const blnIsSelected =
                                    objActiveProposal?.proposalId === objProposal.proposalId;

                                return (
                                    <button
                                        key={objProposal.proposalId}
                                        type="button"
                                        onClick={() => handleSelectProposal(objProposal)}
                                        className={cn(
                                    ...
                                    >
                                        <div className="min-w-0 flex-1 space-y-1">
                                            <div className="flex items-center gap-2">
                                                <h4
                                                    className="truncate text-xs sm:text-sm font-bold text-slate-950"
                                                    title={objProposal.enterpriseName}
                                                >
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-[11px] text-slate-500">
                                                <span className="font-mono font-semibold text-[#285497]">
                                                    {objProposal.referenceNumber}
                                                </span>
                                                <span>•</span>
                                                <span>{objProposal.proponentName}</span>
                                                {objProposal.district && (
                                                    <>
                                                        <span>•</span>
                                                        <span>{objProposal.district}</span>
                                                    </>
                                                )}
                                            </div>
                                            ...
                                            <div className="mt-2 flex items-center gap-2">
                                                <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-200">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${Math.min(100, objProposal.compliancePercentage)}%`,
                                                        }}
                                                    />
                                                </div>
                                                <span className="text-[11px] font-bold text-slate-700 shrink-0">
                                                    {objProposal.compliedCount}/
                                                    {objProposal.totalRequired} (
                                                    {objProposal.compliancePercentage}%)
                                                </span>
                                            </div>
                                        </div>
