  ...
  return (
    <div className="space-y-6">
      <ProposalSectionHeading
        description="Upload clear and complete copies of each required document. You can preview, replace, or remove files before submitting."
        divided={false}
      ...
      <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
        ...
        <p>
          The list below only includes documents the proponent needs to provide
          for {isGia ? 'GIA' : 'SETUP'} screening. DOST evaluation forms and
          remarks are handled after submission.
        </p>
      </div>
        ...
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {data.proposalType
                ? `${requirements.length} documents required for your ${data.proposalType} submission`
