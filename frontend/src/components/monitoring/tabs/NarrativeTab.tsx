/**
 * System: DPRMS
 * Purpose: Render narrative tab for the frontend.
...
/** Render narrative tab and its available actions. */
export function NarrativeTab({ objRecord, onChange, blnReadOnly = false }: Props)
{
    /** Handle update problems. */
    const _handleUpdateProblems = (strField: keyof NarrativeItem, strValue: string) =>
    {
        onChange({
            ...objRecord,
            problemsAndActions: {
                ...objRecord.problemsAndActions,
                [strField]: strValue,
            },
        });
    };
    ...
    /** Handle update plans. */
    const _handleUpdatePlans = (strField: keyof NarrativeItem, strValue: string) =>
    {
        onChange({
            ...objRecord,
            plansForImprovement: {
                ...objRecord.plansForImprovement,
                [strField]: strValue,
            },
        });
    };

    /** Handle sign off change. */
    const _handleSignOffChange = (strField: keyof SignOffMetadata, strValue: string) =>
    {
        onChange({
            ...objRecord,
            signOff: {
    ...
    const arrProblemsDimensions: Array<{ key: keyof NarrativeItem; title: string; }> = [
        {
            key: 'humanResource',
    ...
    const arrPlansDimensions: Array<{ key: keyof NarrativeItem; title: string; }> = [
        {
            key: 'humanResource',
                        ...
                        <thead className="bg-[#E6EEF4] border-b border-[#B5BFCD]/80 text-[11px] font-bold uppercase tracking-wider text-[#285497]">
                            <tr>
                                ...
                                <th className="py-3 px-5">
                                    Problem/s Encountered & Action/s Taken
                                </th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[#B5BFCD]/30 text-slate-800">
                            {arrProblemsDimensions.map((objDim) => (
                                <tr key={objDim.key} className="hover:bg-[#E6EEF4]/30 transition">
                                    ...
                                    <td className="p-3">
                                        <textarea
                                            rows={3}
                                            value={objRecord.problemsAndActions[objDim.key] || ''}
                                            onChange={(objEvent) =>
                                                _handleUpdateProblems(
                                                    objDim.key,
                                                    objEvent.target.value,
                                                )
                                            }
                                            readOnly={blnReadOnly}
                                            placeholder={`Enter specific problems and corrective actions taken for ${objDim.title}...`}
                    ...
                    <table className="w-full text-left text-xs">
                        <thead className="bg-[#E6EEF4] border-b border-[#B5BFCD]/80 text-[11px] font-bold uppercase tracking-wider text-[#285497]">
                            <tr>
                                ...
                                <th className="py-3 px-5">
                                    Target Improvements & Execution Roadmap
                                </th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[#B5BFCD]/30 text-slate-800">
                            {arrPlansDimensions.map((objDim) => (
                                <tr key={objDim.key} className="hover:bg-[#E6EEF4]/30 transition">
                                    <td className="p-4 align-top">
                                        <p className="font-bold text-slate-900 text-xs">
                                            {objDim.title}
                                    ...
                                    <td className="p-3">
                                        <textarea
                                            rows={3}
                                            value={objRecord.plansForImprovement[objDim.key] || ''}
                                            onChange={(objEvent) =>
                                                _handleUpdatePlans(
                                                    objDim.key,
                                                    objEvent.target.value,
                                            ...
                                            readOnly={blnReadOnly}
                                            placeholder={`Enter target execution plans for ${objDim.title}...`}
                                            className="w-full rounded-xl border border-[#B5BFCD] bg-white p-3 text-xs text-slate-900 focus:border-[#285497] focus:ring-1 focus:ring-[#285497] focus:outline-none"
                                        />
                                    </td>
                                </tr>
                        ...
                        <div className="rounded-2xl border border-[#B5BFCD]/80 bg-[#E6EEF4]/40 p-6 space-y-4 text-center">
                            ...
                            <div className="space-y-3 text-xs text-left">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={objRecord.signOff.interviewerName}
                                        onChange={(objEvent) =>
                                            _handleSignOffChange(
                                                'interviewerName',
                                                objEvent.target.value,
                                            )
                                        }
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={objRecord.signOff.interviewerDesignation}
                                        onChange={(objEvent) =>
                                            _handleSignOffChange(
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input
                                            type="date"
                                            value={objRecord.signOff.dateOfVisit}
                                            onChange={(objEvent) =>
                                                _handleSignOffChange(
                                                    'dateOfVisit',
                                                    objEvent.target.value,
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="date"
                                            value={objRecord.signOff.interviewerSignatureDate}
                                            onChange={(objEvent) =>
                                                _handleSignOffChange(
                                ...
                                <div className="h-12 border-b-2 border-dashed border-[#B5BFCD] flex items-end justify-center pb-1">
                                    <span className="font-serif italic text-slate-400 text-sm">
                                        {objRecord.signOff.interviewerName
                                            ? `Signed by ${objRecord.signOff.interviewerName}`
                        ...
                        <div className="rounded-2xl border border-[#B5BFCD]/80 bg-[#E6EEF4]/40 p-6 space-y-4 text-center">
                            ...
                            <div className="space-y-3 text-xs text-left">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={objRecord.signOff.respondentName}
                                        onChange={(objEvent) =>
                                            _handleSignOffChange(
                                                'respondentName',
                                                objEvent.target.value,
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={objRecord.signOff.respondentDesignation}
                                        onChange={(objEvent) =>
                                            _handleSignOffChange(
                                ...
                                <div>
                                    ...
                                    <input
                                        type="date"
                                        value={objRecord.signOff.respondentSignatureDate}
                                        onChange={(objEvent) =>
                                            _handleSignOffChange(
                                ...
                                <div className="h-12 border-b-2 border-dashed border-[#B5BFCD] flex items-end justify-center pb-1">
                                    <span className="font-serif italic text-slate-400 text-sm">
                                        {objRecord.signOff.respondentName
                                            ? `Signed by ${objRecord.signOff.respondentName}`
                                            : 'Signature placeholder'}
                                    </span>
                                </div>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">
