/**
 * System: DPRMS
 * Purpose: Render monitoring overview section for the frontend.
 * Programmer: ITD Development Team
 * Copyright: (c) 2026 ITD. All rights reserved.
 */
import
{
    Activity,
    ArrowRight,
    CheckCircle2,
    ClipboardClock,
    FolderKanban,
    MapPin,
    Store,
} from 'lucide-react';

import type { ProjectRecord } from '../../data/admin';
import type { SetupMonitoringStatistics } from '../../services/setup_monitoring_store';
import { MetricCard } from '../admin/MetricCard';

interface Props
{
    arrProjects: ProjectRecord[];
    objStatistics: SetupMonitoringStatistics;
    onSelectProject: (objProject: ProjectRecord) => void;
    strPeriod?: string;
}

/** Format last monitored. */
function _formatLastMonitored(strValue?: string | null): string
{
    if (!strValue)
    {
        return 'Not yet monitored';
    }

    const dtDate = new Date(strValue);
    if (Number.isNaN(dtDate.getTime()))
    {
        return 'Not yet monitored';
    }

    return dtDate.toLocaleDateString('en-PH', {
        day: 'numeric',
        month: 'short',
        year: 'numeric',
    });
}

/** Render monitoring overview section and its available actions. */
export function MonitoringOverviewSection({
    arrProjects,
    objStatistics,
    onSelectProject,
    strPeriod = 'Current quarter',
}: Props)
{
    const intCoverage =
        objStatistics.activeProjects > 0
            ? Math.round((objStatistics.monitoredCount / objStatistics.activeProjects) * 100)
            : 0;
    const arrNeedsAttention = arrProjects
        .filter((objProject) => !objProject.monitored || (objProject.pendingReports ?? 0) > 0)
        .slice(0, 5);

    const arrCards: Array<{
        detail: string;
        icon: typeof FolderKanban;
        label: string;
        tone: 'blue' | 'green' | 'sky' | 'orange';
        value: string | number;
    }> = [
            {
                label: 'Active SETUP projects',
                value: objStatistics.activeProjects,
                detail: 'Approved projects currently under implementation',
                icon: FolderKanban,
                tone: 'blue',
            },
            {
                label: 'Monitored count',
                value: objStatistics.monitoredCount,
                detail: 'Verified monitoring activities conducted',
                icon: CheckCircle2,
                tone: 'green',
            },
            {
                label: 'Monitoring coverage',
                value: `${intCoverage}%`,
                detail: `${objStatistics.monitoredCount} of ${objStatistics.activeProjects} active projects`,
                icon: Activity,
                tone: 'sky',
            },
            {
                label: 'Pending reports',
                value: objStatistics.pendingReports,
                detail:
                    objStatistics.pendingReports === 0
                        ? 'No draft or returned reports require action'
                        : 'Draft or returned reports requiring action',
                icon: ClipboardClock,
                tone: 'orange',
            },
        ];

    return (
        <div className="space-y-5 font-sans">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {arrCards.map((objCard) => (
                    <MetricCard
                        strDetail={objCard.detail}
                        icon={objCard.icon}
                        key={objCard.label}
                        strLabel={objCard.label}
                        strTone={objCard.tone}
                        value={objCard.value}
                    />
                ))}
            </div>

            <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.55fr)]">
                <section className="overflow-hidden rounded-2xl border border-[#B5BFCD]/70 bg-white shadow-sm">
                    <div className="flex items-center justify-between gap-4 border-b border-[#B5BFCD]/50 px-5 py-4">
                        <div>
                            <h2 className="text-base font-bold text-slate-950">
                                Projects requiring attention
                            </h2>
                            <p className="mt-0.5 text-xs text-slate-500">
                                Unmonitored projects and projects with pending reports for{' '}
                                {strPeriod}.
                            </p>
                        </div>
                        <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">
                            {arrNeedsAttention.length} shown
                        </span>
                    </div>

                    {arrNeedsAttention.length === 0 ? (
                        <div className="flex min-h-52 flex-col items-center justify-center px-6 text-center">
                            <span className="flex size-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700">
                                <CheckCircle2 className="size-6" />
                            </span>
                            <h3 className="mt-3 text-sm font-bold text-slate-900">
                                Monitoring is up to date
                            </h3>
                            <p className="mt-1 text-xs text-slate-500">
                                No active SETUP projects currently need attention.
                            </p>
                        </div>
                    ) : (
                        <div className="divide-y divide-[#B5BFCD]/40">
                            {arrNeedsAttention.map((objProject) => (
                                <button
                                    key={objProject.id}
                                    type="button"
                                    onClick={() => onSelectProject(objProject)}
                                    className="group flex w-full items-center gap-3 px-5 py-4 text-left transition hover:bg-[#E6EEF4]/45"
                                >
                                    <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#E6EEF4] text-[#285497]">
                                        <Store className="size-4.5" />
                                    </span>
                                    <span className="min-w-0 flex-1">
                                        <span className="block truncate text-sm font-bold text-slate-900">
                                            {objProject.enterprise || objProject.title}
                                        </span>
                                        <span className="mt-1 flex items-center gap-1 truncate text-xs text-slate-500">
                                            <MapPin className="size-3 shrink-0" />
                                            {objProject.location || 'Location not recorded'}
                                        </span>
                                    </span>
                                    <span className="hidden text-right sm:block">
                                        <span className="block text-xs font-bold text-slate-700">
                                            {(objProject.pendingReports ?? 0) > 0
                                                ? `${objProject.pendingReports} pending`
                                                : 'Monitoring needed'}
                                        </span>
                                        <span className="mt-0.5 block text-[11px] text-slate-400">
                                            {_formatLastMonitored(objProject.lastMonitoredAt)}
                                        </span>
                                    </span>
                                    <ArrowRight className="size-4 shrink-0 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-[#0f53b7]" />
                                </button>
                            ))}
                        </div>
                    )}
                </section>

                <aside className="rounded-2xl border border-[#B5BFCD]/70 bg-white p-5 shadow-sm">
                    <p className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500">
                        Monitoring coverage
                    </p>
                    <div className="mt-6 flex justify-center">
                        <div
                            className="grid size-40 place-items-center rounded-full"
                            style={{
                                background: `conic-gradient(#0f53b7 ${intCoverage * 3.6}deg, #e6eef4 0deg)`,
                            }}
                        >
                            <div className="grid size-28 place-items-center rounded-full bg-white text-center shadow-inner">
                                <div>
                                    <p className="text-3xl font-black text-slate-950">
                                        {intCoverage}%
                                    </p>
                                    <p className="text-[11px] font-semibold text-slate-500">
                                        covered
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="mt-6 rounded-xl bg-[#E6EEF4]/60 px-4 py-3 text-center">
                        <p className="text-sm font-bold text-slate-900">
                            {objStatistics.monitoredCount} of {objStatistics.activeProjects}
                        </p>
                        <p className="mt-0.5 text-xs text-slate-500">active projects monitored</p>
                    </div>
                </aside>
            </div>
        </div>
    ); // end return
} /* end MonitoringOverviewSection */
