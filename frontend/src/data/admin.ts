/**
 * System: DPRMS
 * Purpose: Admin definitions for DPRMS.
...
export interface PredictionRecord
{
    ...
    riskScore: number;
    sustainability: 'Sustainable' | 'Moderately sustainable' | 'Unsustainable';
}

export const PROPOSAL_RECORDS: ProposalRecord[] = [
    {
        id: 'PR-2026-041',
...
export const PROJECT_RECORDS: ProjectRecord[] = [
    ...
    },
    {
        ...
        gia: {
            ...
            cooperatingAgencies: ['Municipality of Cateel', 'Davao Oriental State University'],
            durationMonths: 12,
            endDate: 'Feb 28, 2027',
...
/** Format currency. */
export function formatCurrency(intValue: number): string
{
...
export function getProposalReviewStatus(objProposal: ProposalRecord): ProposalReviewStatus
{
    if (objProposal.status === 'Approved')
    {
        return 'Approved';
    }
    if (objProposal.status === 'Rejected')
    {
        return 'Disapproved';
    }

    const objStageStatus: Record<ProposalRecord['stage'], ProposalReviewStatus> = {
        0: 'Submitted',
    ...
    return objStageStatus[objProposal.stage];
}
